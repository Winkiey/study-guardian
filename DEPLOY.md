...
**推送失败时一定要看这个文件** —— 之前遇到过一次「明明推了、服务器上却没变化」，
原因就是推送被 GitHub 拒绝了，而那个错误在命令行窗口里一闪就没了。
看到窗口结尾是 `Done. Your code is on GitHub.` 才算真的成功。

> 这个脚本只管**推送**，不管**提交**。提交（`git commit`）是改代码那一步做的事。
> 所以：**没有先提交的改动，是推不上去的** —— 你在电脑上手动改过的文件，
> 如果没有提交，双击这个脚本只会推走已有的提交，你的改动还留在本地。
...
网页端的「设置 → 注销账号」要求输入**原密码**（防误删，这个设计是对的）。
但有些场合密码是不知道的，比如：

- 做安全测试的人建了一个临时账号，没保存密码；
- 有人试注册留下的账号，你想清掉。
...
```bash
cd /opt/study-guardian
node scripts/delete-user.mjs                    # 先列出所有账号，什么也不动
node scripts/delete-user.mjs 要删的用户名        # 先给你看会删掉什么，再问你一次
```

确认那一步要**手动敲 `yes`**；如果是在脚本里跑（没有交互终端），必须显式加 `--yes`：

```bash
node scripts/delete-user.mjs 要删的用户名 --yes
```

它和网页端注销走的是**同一段代码**（`src/lib/account.js`），所以删得一样干净：
数据库里的课程、作业、提醒、通知渠道、发送日志、设置，
以及磁盘上的课件原件、转换出来的 PDF、导出的幻灯片图片。

> ⚠️ **不可撤销。** 删之前如果想留一手，先按上面那节打个备份。
> 脚本在删除前会把「将删掉几个课件、几个文件」列出来，看清楚再确认。
...
密码就是唯一的门。注册时最少 8 位，但**建议 16 位以上、别和其它网站重复**。
最省事的做法是用一句你记得住的话，比如 `dufu2026shici!`。
...
用户名逐字节唯一，而且**区分大小写**：`Winkie` 和 `winkie` 是两个可以各自
注册的名字。只有完全一模一样才会撞车，换个名字就行（提示里会写清是哪个名字被占了）。

> 反过来也要提醒同学：注册时怎么拼的，以后登录就得怎么拼 ——
> 差一个字母的大小写就是「用户名或密码不正确」。
